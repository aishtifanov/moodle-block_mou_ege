<?PHP // $Id: stats_ege_region.php,v 1.8 2009/06/11 09:40:36 Shtifanov Exp $
    ...
    $strdisciplines = get_string('stats_ege_region', 'block_mou_ege');
    ...
    foreach ($rayons as $rayon)		{
	    ...
	    if ($pupils)	{
			...
			$tabledata = array ($g.'.', '<b>'.$rayon->name.'</b>');
			foreach ($disciplines as $discipline) 	{
			...
			$table->data[] = $tabledata;
			 ...
			 foreach ($schools as $sa)  {
			    $strsql = "SELECT id, userid, classid, schoolid, listegeids
			    		   FROM {$CFG->prefix}monit_school_pupil_card
						   WHERE schoolid = {$sa->id} AND deleted=0";
				// echo $strsql;
			    ...
			    if ($pupils)	{
					...
					$tabledata = array ($g.'.'.$sa->number.'.', '<i>'.$sa->name.'</i>');
					foreach ($disciplines as $discipline) 	{
		                $tabledata[] = $ass_count[$discipline->id];
					}
					$table->data[] = $tabledata;

			    }   else {					$tabledata = array ($g.'.'.$sa->number.'.', '<i><u>'.$sa->name.'</i></u>');
					foreach ($disciplines as $discipline) 	{
		                $tabledata[] = '-';
					}
					$table->data[] = $tabledata;
			...
			$tabledata = array ($g.'.', '<b><u>'.$rayon->name.'</u></b>');
			foreach ($disciplines as $discipline) 	{
                $tabledata[] = '-';
			}
			$table->data[] = $tabledata;
...
<table align="center">
	<tr>
	<td>
	<form name="download" method="post" action="<?php echo "stats_ege_region.php?action=excel&amp;yid=$yid" ?>">
	    <div align="center">
...
function disciplines_ege_download($yid)
{
    global $CFG;
    ...
    $txtl = new textlib();
        ...
        $downloadfilename = "stats_giu_region";
			 ...
			 foreach ($schools as $sa)  {
			    ...
			    if ($pupils)	{
			        ...
			        foreach ($arr_disc_id as $disc_id)	{
			 	    ...
			 	    $myxls->write_string($ii, 0, $g.'.'.$sa->number.'.', $formatp);
               		$strwin1251 =  $txtl->convert($sa->name, 'utf-8', 'windows-1251');
           		    $myxls->write_string($ii, 1, $strwin1251, $formatpl);

                    $jj = 2;
					foreach ($disciplines as $discipline) 	{
						 $myxls->write($ii, $jj++, $ass_count[$discipline->id], $formatp);
					}

<?php // $Id: importclasses.php,v 1.18 2009/11/12 10:47:54 Shtifanov Exp $
    ...
    $strpupil = get_string('importclass', 'block_mou_att');
...
//	if (!empty($frm) ) {
		...
		if ($um->preprocess_files()) {
			...
			$text = file($filename);
			if($text == FALSE)	{
				error(get_string('errorfile', 'block_monitoring'), "$CFG->wwwroot/blocks/mou_ege/class/importclasses.php?sid=$sid&amp;rid=$rid&amp;yid=$yid");
			}
			$size = sizeof($text);
			  ...
			  foreach ($symbols as $sym)	{
				...
				if(!$class = get_record('monit_school_class', 'schoolid', $sid, 'yearid', $yid, 'name', $pupil->name)) 	 {
						...
						if ($idnew = insert_record('monit_school_class', $rec))	{
						    ...
						    print_r($rec); echo '<hr>';
							error(get_string('errorinaddingclass','block_mou_ege'), "$CFG->wwwroot/blocks/mou_ege/class/importclasses.php?sid=$sid&amp;rid=$rid&amp;yid=$yid");
						}
                }
...
//   }
    if ($admin_is  || $region_operator_is)	 {
		...
		listbox_rayons("importclasses.php?sid=0&amp;yid=$yid&amp;rid=", $rid);
		listbox_schools("importclasses.php?rid=$rid&amp;yid=$yid&amp;sid=", $rid, $sid, $yid);
		echo '</table>';
	...
	if ($rid!=0 && $sid!=0) {
	    ...
	    $currenttab = 'importclass';
 	    include('tabsclasses.php');

	    $struploadusers = get_string('importclasspupil', 'block_mou_ege', $strschool);
	    print_heading_with_help($struploadusers, 'importclass', 'mou');

		/// Print the form
   	    $struploadusers = get_string('importclass', 'block_mou_ege');
	    ...
	    echo '<input type="file" name="userfile" size="50">'.
	         ...
	         '</form><p>';
	    $output = helpbutton('importclass', 'Как загрузить списки учеников нескольких классов', 'mou', true, true, '', true);
	    echo $output;
	    echo '</center>';
    }

<?php // $Id: import9classes.php,v 1.8 2009/11/12 10:47:54 Shtifanov Exp $
...
//	if (!empty($frm) ) {
		...
		if ($um->preprocess_files()) {
			...
			$text = file($filename);
			if($text == FALSE)	{
				error(get_string('errorfile', 'block_monitoring'), "$CFG->wwwroot/blocks/mou_ege/class/import9classes.php?sid=$sid&amp;rid=$rid&amp;yid=$yid");
			}
			$size = sizeof($text);
            ...
            foreach ($header as $i => $h) {
                ...
                $header[$i] = $h;
                if (!isset($required[$h])) {
                    error(get_string('invalidfieldname', 'error', $string_rus[$h]), "$CFG->wwwroot/blocks/mou_ege/class/import9classes.php?sid=$sid&amp;rid=$rid&amp;yid=$yid");
                }
                if (isset($required[$h])) {
                    $required[$h] = 0;
                }
               ...
               // echo $pupil->when_hands.':';
				...
				if(!$class = get_record('monit_school_class', 'schoolid', $sid, 'yearid', $yid, 'name', $pupil->name)) 	 {
						...
						if ($idnew = insert_record('monit_school_class', $rec))	{
						    ...
						    print_r($rec); echo '<hr>';
							error(get_string('errorinaddingclass','block_mou_ege'), "$CFG->wwwroot/blocks/mou_ege/class/import9classes.php?sid=$sid&amp;rid=$rid&amp;yid=$yid");
						}
                    }

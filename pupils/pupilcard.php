<?php // $Id: pupilcard.php,v 1.20 2011/04/21 06:51:59 shtifanov Exp $
	...
	if ($numclass == 9)	{
		// if (!$admin_is && !$region_operator_is && ($rayon_operator_is || $school_operator_is))	{
		   if (!$CFG->monit_open_close_9)   {
			// $onclickreturnfalse = 'onclick="return false;"';
			// redirect($CFG->wwwroot."/blocks/mou_ege/pupils/pupil.php?mode=4&amp;rid=$rid&amp;sid=$sid&amp;yid=$yid&amp;gid=$gid&amp;uid=$uid",'',0);
			 notice(get_string('accesstemporarylock', 'block_mou_ege'), $CFG->wwwroot."/blocks/mou_ege/pupils/pupil.php?mode=4&amp;rid=$rid&amp;sid=$sid&amp;yid=$yid&amp;gid=$gid&amp;uid=$uid");
           }  
		// }
	}	
       ...
       */
        ...
        */
        ...
        if (find_regform_errors($user, $usernew, $err, $um, $profile)) {
			...
			if ($disciplines)	{
				...
				foreach ($disciplines as $discipline) 	{
					...
					if (isset($usernew->{$pf}) && $usernew->{$pf} == 1)  {
						...
						$pfdate = 'date_ege_'.$discipline->id;
						$listdatesids .= $usernew->{$pfdate} . ',';
					}
				}
				$listegeids .= '0';
				$pupil->listegeids = $listegeids;
				$listdatesids .= '0';
				$pupil->listdatesids = $listdatesids;
			}
			...
			$pupil1->listegeids = $pupil->listegeids;
			$pupil1->listmiids = $pupil->listmiids;
			$pupil1->listdatesids = $pupil->listdatesids;
			$pupil1->listmidatesids = $pupil->listmidatesids;
            ...
            if (update_record("user", $usernew)) {
                ...
                if ($user->id == $USER->id) {
                    ...
                    if (!empty($SESSION->wantsurl)) {  // User may have been forced to edit account, so let's
                        ...
                        redirect("$CFG->wwwroot/blocks/mou_ege/class/classpupils.php?rid=$rid&amp;sid=$sid&amp;yid=$yid&amp;gid=$gid", get_string("changessaved"), 0);
                    }
                } else {
                    redirect("$CFG->wwwroot/blocks/mou_ege/class/classpupils.php?rid=$rid&amp;sid=$sid&amp;yid=$yid&amp;gid=$gid", get_string("changessaved"), 0);
                }
